...
$imageByItemCodeLower = array(); // ✅ ໃຊ້ເປັນ fallback ກໍລະນີ Item_code ໃນ DB ແລະໃນ filemap.json ຕົວພິມໃຫຍ່/ນ້ອຍບໍ່ກົງກັນ
foreach ($imageMap as $safeName => $itemCode) {
	// ✅ ຕັດຊ່ອງຫວ່າງໜ້າ-ຫຼັງອອກ ເພື່ອກັນກໍລະນີ Item_code ໃນ filemap.json ມີ space ຕິດມາ
	// ເຮັດໃຫ້ບໍ່ກົງກັບ Item_code ໃນ DB (ຮູບຈຶ່ງບໍ່ຂຶ້ນທັ້ງທີ່ມີໄຟລ໌ຮູບຢູ່ແທ້ໆ)
	...
	if (!isset($imageByItemCode[$itemCode]) || (file_exists($path) && filemtime($path) > (isset($imageByItemCode[$itemCode]['mtime']) ? $imageByItemCode[$itemCode]['mtime'] : 0))) {
		...
		$imageByItemCode[$itemCode] = $entry;
		$imageByItemCodeLower[mb_strtolower($itemCode)] = $entry;
	}
}
 ...
 */
function getItemImageUrl($itemCode, $imageByItemCode, $uploadsBaseDir, $uploadsBaseUrl, $imageByItemCodeLower = array())
{
	$itemCode = trim((string) $itemCode);
	if ($itemCode === '') {
		return null;
	...
	if (isset($imageByItemCode[$itemCode])) {
		$safeName = $imageByItemCode[$itemCode]['safeName'];
	} elseif (isset($imageByItemCodeLower[mb_strtolower($itemCode)])) {
		// ✅ fallback: ບໍ່ພົບແບບກົງເປັະ ແຕ່ພົບແບບບໍ່ສົນໃຈໂຕພິມໃຫຍ່/ນ້ອຍ
		$safeName = $imageByItemCodeLower[mb_strtolower($itemCode)]['safeName'];
	} else {
		return null;
	}
	...
	<tbody>
		...
		if ($result23 instanceof mysqli_result) {
			while ($row23 = $result23->fetch_assoc()) {
				...
				$itemImageUrl = getItemImageUrl($row23['Item_code'] ?? '', $imageByItemCode, $uploadsBaseDir, $uploadsBaseUrl, $imageByItemCodeLower);
		?>
				<tr bgcolor="<?php echo $bgcolor ?>" style="font-size: 13px;">
					...
					<td width="20%">
						...
						<?php else:
							$dbgCode = trim((string)($row23['Item_code'] ?? ''));
							if ($dbgCode === '') {
								$dbgReason = 'Item_code ວ່າງເປົ່າ';
							} elseif (isset($imageByItemCode[$dbgCode])) {
								$dbgReason = 'ພົບໃນ filemap.json ແຕ່ໄຟລ໌ຮູບບໍ່ຢູ່ໃນ picture/uploads/ ແລ້ວ (safeName: ' . $imageByItemCode[$dbgCode]['safeName'] . ')';
							} elseif (isset($imageByItemCodeLower[mb_strtolower($dbgCode)])) {
								$dbgReason = 'ພົບແບບບໍ່ກົງໂຕພິມໃຫຍ່/ນ້ອຍ ແຕ່ໄຟລ໌ບໍ່ຢູ່';
							} else {
								$dbgReason = 'ບໍ່ພົບ Item_code "' . $dbgCode . '" ໃນ filemap.json ເລີຍ (ຍັງບໍ່ເຄີຍອັບໂຫຼດຮູບໃຫ້ລະຫັດນີ້)';
							}
						?>
							<span class="text-muted">-</span>
							<div style="font-size:10px;color:#c0392b;line-height:1.3;margin-top:2px;"><?php echo htmlspecialchars($dbgReason) ?></div>
						<?php endif; ?>
					</td>
				</tr>
		<?php
