			...
			?>
					<tr bgcolor="<?= $showcolor; ?>" style="font-size: 13px;">
						...
						<td width="20%"><strong> <i class="fa-light fa-memo-circle-info fa-lg"></i> PR ເລກທີ:
								<span class="pr-display" id="prDisplay<?= $id ?>">
									<?php if (!empty($row['OA_Out'])): ?>
										<a href="pr_detail.php?pr_no=<?php echo urlencode($row['OA_Out']); ?>" target="_blank" class="pr-link" style="color: #d32f2f; text-decoration: underline; font-weight: bold;">
											<?php echo htmlspecialchars($row['OA_Out']); ?>
										</a>
									<?php else: ?>
										<span class="pr-link">-</span>
									<?php endif; ?>
								</span>
								<input type="text" class="pr-input" id="prInput<?= $id ?>" data-original="<?php echo htmlspecialchars($row['OA_Out']); ?>" style="display:none; width:120px; font-size:12px;" value="<?php echo htmlspecialchars($row['OA_Out']); ?>" />
								<a href="#" class="pr-edit-btn" data-id="<?= $id ?>" title="ແກ້ໄຂ PR ເລກທີ"><i class="fa fa-pencil"></i></a>
								<a href="#" class="pr-save-btn" data-id="<?= $id ?>" style="display:none;" title="ບັນທຶກ"><i class="fa fa-check text-success"></i></a>
								<a href="#" class="pr-cancel-btn" data-id="<?= $id ?>" style="display:none;" title="ຍົກເລີກ"><i class="fa fa-times text-danger"></i></a>
...
ORDER BY a.dateRe asc ";
	...
	<script>
		// ✅ ແກ້ໄຂ PR ເລກທີ ແບບ inline (ບໍ່ຕ້ອງໂຫຼດໜ້າໃໝ່)
		document.addEventListener('click', function(e) {
			var editBtn = e.target.closest('.pr-edit-btn');
			var saveBtn = e.target.closest('.pr-save-btn');
			var cancelBtn = e.target.closest('.pr-cancel-btn');

			if (editBtn) {
				e.preventDefault();
				var id = editBtn.getAttribute('data-id');
				document.getElementById('prDisplay' + id).style.display = 'none';
				var input = document.getElementById('prInput' + id);
				input.style.display = 'inline-block';
				input.focus();
				input.select();
				editBtn.style.display = 'none';
				document.querySelector('.pr-save-btn[data-id="' + id + '"]').style.display = 'inline-block';
				document.querySelector('.pr-cancel-btn[data-id="' + id + '"]').style.display = 'inline-block';
			}
			...
			if (cancelBtn) {
				...
				document.getElementById('prDisplay' + id).style.display = '';
				document.querySelector('.pr-edit-btn[data-id="' + id + '"]').style.display = 'inline-block';
				cancelBtn.style.display = 'none';
				document.querySelector('.pr-save-btn[data-id="' + id + '"]').style.display = 'none';
			}
			...
			if (saveBtn) {
				...
				var cancelEl = document.querySelector('.pr-cancel-btn[data-id="' + id + '"]');
				...
				fetch('update_pr.php', {
						...
						body: 'id=' + encodeURIComponent(id) + '&OA_Out=' + encodeURIComponent(newVal)
					})
					.then(function(res) {
						return res.json();
					})
					.then(function(data) {
						var display = document.getElementById('prDisplay' + id);
						var editEl = document.querySelector('.pr-edit-btn[data-id="' + id + '"]');

						if (data.success) {
							if (newVal) {
								var a = document.createElement('a');
								a.href = 'pr_detail.php?pr_no=' + encodeURIComponent(newVal);
								a.target = '_blank';
								a.className = 'pr-link';
								a.style.cssText = 'color:#d32f2f;text-decoration:underline;font-weight:bold;';
								a.textContent = newVal;
								display.innerHTML = '';
								display.appendChild(a);
							} else {
								display.innerHTML = '<span class="pr-link">-</span>';
							}
							input.setAttribute('data-original', newVal);
						} else {
							alert('ບັນທຶກບໍ່ສຳເລັດ: ' + (data.message || 'ເກີດຂໍ້ຜິດພາດ'));
							input.value = input.getAttribute('data-original');
						}
		...
		document.addEventListener('keydown', function(e) {
			if (e.target.classList && e.target.classList.contains('pr-input')) {
				if (e.key === 'Enter') {
					e.preventDefault();
					var id = e.target.id.replace('prInput', '');
					document.querySelector('.pr-save-btn[data-id="' + id + '"]').click();
				} else if (e.key === 'Escape') {
					e.preventDefault();
					var id = e.target.id.replace('prInput', '');
					document.querySelector('.pr-cancel-btn[data-id="' + id + '"]').click();
				}
			}
		});
	</script>
