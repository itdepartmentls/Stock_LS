...
/* ==========================================================================
   ຮູບອຸປະກອນ (Equipment picture) - ໃຊ້ pattern ດຽວກັນກັບ allstock.php:
   ອ່ານ picture/uploads/filemap.json (safeName => Item_code) ຈັບຄູ່ Item_code
   ຖ້າພົບໄຟລ໌ຈິງໃນ picture/uploads/ ໃຫ້ໃຊ້ຮູບນັ້ນ, ຖ້າບໍ່ພົບ fallback ໄປໃຊ້ column
   picture ຈາກ DB (stock.picture)
   ========================================================================== */
...
<head>
	...
	<style type="text/css">
		...
		/* ===================== ຮູບອຸປະກອນ (Equipment thumbnail) ===================== */
		.equip-thumb {
			width: 60px;
			height: 60px;
			object-fit: cover;
...
b.Unit 'Unitpro',
c.Unit 'Unitcenter',
(SELECT GROUP_CONCAT(DISTINCT t.Number_Bin SEPARATOR ', ') FROM tacking t WHERE t.OA = a.OA_Out) AS TackingDocs
					...
					<table class="table circle table-bordered">
						<thead>
							<tr class="cll" align="center" style="font-size: 15px;color:mediumblue">
								...
								<th width="8%" data-translate="picture">ຮູບອຸປະກອນ</th>
							...
							?>
								<tr style="font-size: 13px;">
									...
									<td width="18%">
										...
										<i class="fa-light fa-memo-circle-info fa-lg"></i> <strong data-translate="prNumber">PR ເລກທີ:</strong> <a href="tracking_Data_Re_addmin.php?pr=<?php echo urlencode($row['OA_Out']) ?>&lang=<?php echo urlencode($lang) ?>" target="_blank" title="ເບິ່ງການຕິດຕາມ PR ນີ້"><?php echo htmlspecialchars($row['OA_Out']) ?></a>
										<?php if (!empty($row['TackingDocs'])): ?>
											<br><span class="badge rounded-pill bg-danger text-white" title="ເລກທີ່ໃບຂົນສົ່ງ: <?php echo htmlspecialchars($row['TackingDocs']) ?>">
												<i class="fa-solid fa-truck-fast"></i> ອອກໃບຂົນສົ່ງແລ້ວ (<?php echo htmlspecialchars($row['TackingDocs']) ?>)
											</span>
										<?php endif; ?>
										<br>
									...
									<td width="8%" align="center">
										<?php $picUrl = getStockImage($row['Item_code'] ?? '', $imageMap, $pictureDir, $pictureUrl, ''); ?>
										<?php if ($picUrl !== ''): ?>
											<a href="<?php echo htmlspecialchars($picUrl) ?>" target="_blank" title="ເບິ່ງຮູບເຕັມ">
												<img src="<?php echo htmlspecialchars($picUrl) ?>" alt="<?php echo htmlspecialchars($row['Items']) ?>" class="equip-thumb" onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='inline';">
												<span class="text-muted" style="display:none;font-size:11px;" data-translate="noPicture">ບໍ່ມີຮູບ</span>
											</a>
										<?php else: ?>
											<span class="text-muted" style="font-size:11px;" data-translate="noPicture">ບໍ່ມີຮູບ</span>
										<?php endif; ?>
									</td>
