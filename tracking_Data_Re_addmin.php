...
// ---- ຮັບຄ່າ PR ເລກທີ ຈາກ URL (ຕົວຢ່າງ: ລິ້ງມາຈາກ DataReAdmin.php?...&pr=LS-OFFICE-368) ----
// ໃຊ້ສະເພາະຕອນ "ສ້າງໃໝ່" (ບໍ່ແມ່ນຕອນ ?edit=...) ເພື່ອຊ່ວຍໂຫລດລາຍການສິນຄ້າຂອງ PR ນີ້ອັດຕະໂນມັດ
$prParam = isset($_GET['pr']) ? trim($_GET['pr']) : '';
...
<body class="pb-10">
    ...
    <script>
        ...
        // ★ PR ເລກທີທີ່ສົ່ງມາທາງ URL (?pr=...) ຈາກໜ້າ DataReAdmin.php ເພື່ອໂຫລດລາຍການອັດຕະໂນມັດ
        window.INITIAL_PR = <?php echo json_encode($prParam, JSON_UNESCAPED_UNICODE); ?>;
        ...
        function loadEditData() {
            if (!window.EDIT_DATA) {
                // ຖ້າມີ PR ສົ່ງມາທາງ URL (?pr=...) ໃຫ້ໂຫລດລາຍການ PR ນັ້ນອັດຕະໂນມັດ ຫຼັງຈາກໂຫລດລາຍຊື່ PR ແລ້ວ
                loadPrList(window.INITIAL_PR || null).then(function() {
                    if (window.INITIAL_PR) {
                        const prInput = document.getElementById('pr-no');
                        if (prInput) prInput.value = window.INITIAL_PR;
                        onPrSelected();
                    }
                });
                return;
