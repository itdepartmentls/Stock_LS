...
<div class="container py-5">
  ...
  <div class="row g-4">
    ...
    // ກວດຮູບເກົ່າທີ່ຍັງບໍ່ຢູ່ໃນ filemap → rename ເປັນຊື່ ASCII ທັນທີ
    $mapChanged = false;
    ...
    foreach ($allFiles as $oldFile) {
        if (!in_array($oldFile, $mapKeys)) {
            ...
            if (rename($oldPath, $newPath)) {
                $map[$newSafe] = $oldFile; // ເກັບຊື່ເດີມໄວ້ສະແດງ
                $mapChanged = true;
            }
        }
    }
    // ບັນທຶກ filemap ຖ້າມີການ rename
    if ($mapChanged) {
        file_put_contents($mapFile, json_encode($map));
    }
    ...
    if (empty($map)) {
        ...
        foreach ($map as $safeName => $originalName) {
            ...
            echo '<img src="' . htmlspecialchars($dir . $safeName) . '" class="card-img-top" style="object-fit: cover; height: 200px;">';
