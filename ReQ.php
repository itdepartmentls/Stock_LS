...
<head>
	...
	// ===== ລາຍຊື່ "ຈັດຊື້" (ຜູ້ຮັບຜິດຊອບ) ດຶງຈາກ tb_by_to ສຳລັບ dropdown ຫຼັງແຖບ ໝາຍເຫດ/Remark =====
	$byToList = [];
	$byToRes = $conn->query("SELECT id, name_of_detail_by FROM tb_by_to ORDER BY id ASC");
	if ($byToRes) {
		while ($byToRow = $byToRes->fetch_assoc()) {
			$byToList[] = $byToRow;
		}
	}
	...
	// ===== ເລກ PR ອັດຕະໂນມັດ ສະເພາະ factory = Vientiane (LS-OFFICE xxx) =====
	// ໝາຍເຫດ: ຕອນສະແດງໜ້າ (GET) ຈະ "ເບິ່ງລ່ວງໜ້າ" ເລກຕໍ່ໄປ ໂດຍ*ບໍ່ບັນທຶກ/ບໍ່ຂຶ້ນເລກ*
	// ເລກຈະຖືກຂຶ້ນແທ້ (ບັນທຶກລົງໄຟລ໌) ສະເພາະຕອນກົດ submit ສຳເລັດເທົ່ານັ້ນ (ເບິ່ງຢູ່ລຸ່ມສຸດ ບ່ອນ $conn->commit())
	// ຖ້າມີເລກ PR ຈິງຢູ່ໃນຖານຂໍ້ມູນ/ລະບົບ pr_detail.php ແລ້ວ ຄວນປ່ຽນມາອ່ານຈາກບ່ອນນັ້ນແທນ.
	$isVientiane = isset($_SESSION['factory']) && strcasecmp(trim($_SESSION['factory']), 'Vientiane') === 0;
	$officeCounterFile = __DIR__ . '/ls_office_counter.txt';
	$autoOfficeNumber = '';
	if ($isVientiane) {
		$current = file_exists($officeCounterFile) ? (int) trim(file_get_contents($officeCounterFile)) : 373;
		if ($current <= 0) $current = 373;
		$peekNext = $current + 1;
		$autoOfficeNumber = 'LS-OFFICE ' . $peekNext;
	}

	// ===== ເລກ PR ອັຕະໂນມັດ ສະເພາະ userlogin = HR-Sanakham (SNKHR001, SNKHR002, ...) =====
	// ໝາຍເຫດ: ຕອນສະແດງໜ້າ (GET) ຈະ "ເບິ່ງລ່ວງໜ້າ" ເລກຕໍ່ໄປ ໂດຍ*ບໍ່ບັນທຶກ/ບໍ່ຂຶ້ນເລກ*
	// ເລກຈະຖືກຂຶ້ນແທ້ (ບັນທຶກລົງໄຟລ໌) ສະເພາະຕອນກົດ submit ສຳເລັດເທົ່ານັ້ນ (ເບິ່ງຢູ່ລຸ່ມສຸດ ບ່ອນ $conn->commit())
	$isHrSanakham = isset($_SESSION['factory']) && strcasecmp(trim($_SESSION['factory']), 'HR-Sanakham') === 0;
	$hrCounterFile = __DIR__ . '/snkhr_counter.txt';
	if ($isHrSanakham) {
		$currentHr = file_exists($hrCounterFile) ? (int) trim(file_get_contents($hrCounterFile)) : 0;
		if ($currentHr < 0) $currentHr = 0;
		$peekNextHr = $currentHr + 1;
		$autoOfficeNumber = 'SNKHR' . sprintf('%03d', $peekNextHr);
	}

	// ===== ເລກ PR ອັຕະໂນມັດ ສະເພາະ userlogin = HR-Luangphabang (MMNHR001, MMNHR002, ...) =====
	// ໝາຍເຫດ: ຕອນສະແດງໜ້າ (GET) ຈະ "ເບິ່ງລ່ວງໜ້າ" ເລກຕໍ່ໄປ ໂດຍ*ບໍ່ບັນທຶກ/ບໍ່ຂຶ້ນເລກ*
	// ເລກຈະຖືກຂຶ້ນແທ້ (ບັນທຶກລົງໄຟລ໌) ສະເພາະຕອນກົດ submit ສຳເລັດເທົ່ານັ້ນ (ເບິ່ງຢູ່ລຸ່ມສຸດ ບ່ອນ $conn->commit())
	// ຮອງຮັບທັງ 'HR-Luangphab' (ຄ່າຈິງທີ່ພົບໃນຖານຂໍ້ມູນ) ແລະ 'HR-Luangphabang' ເຜື່ອຄ່າຖືກແກ້ໄຂພາຍຫຼັງ
	$isHrLuangphabang = isset($_SESSION['factory']) && (
		strcasecmp(trim($_SESSION['factory']), 'HR-Luangphab') === 0 ||
		strcasecmp(trim($_SESSION['factory']), 'HR-Luangphabang') === 0
	);
	$mmnCounterFile = __DIR__ . '/mmnhr_counter.txt';
	if ($isHrLuangphabang) {
		$currentMmn = file_exists($mmnCounterFile) ? (int) trim(file_get_contents($mmnCounterFile)) : 0;
		if ($currentMmn < 0) $currentMmn = 0;
		$peekNextMmn = $currentMmn + 1;
		$autoOfficeNumber = 'MMNHR' . sprintf('%03d', $peekNextMmn);
	}

	// ===== ກຸ່ມ Sanakham / HR-Sanakham: dropdown "ຈັດຊື້" ໃຫ້ສະແດງ 2 ຕົວເລືອກຄົງທີ່ນີ້
	// (ຊື້ຢູ່ນະຄອນຫຼວງວຽງຈັນ / ຊື້ຢູ່ປາກລາຍ) ແທນລາຍຊື່ຈາກ tb_by_to =====
	// ໝາຍເຫດ: ຄ່າ "ໂຮງງານ" ທີ່ສະແດງເທິງໜ້ານີ້ ('ສຳນັກງານໃຫຍ່/ໂຮງງານ') ຈິງໆແລ້ວມາຈາກ
	// $_SESSION['Namepro'] ບໍ່ແມ່ນ $_SESSION['factory'] ຈຶ່ງກວດທັງສອງບ່ອນ ເພື່ອບໍ່ໃຫ້ພາດ
	// (ຄ່າ 'factory' ອາດບໍ່ໄດ້ຖືກຕັ້ງໄວ້ສຳລັບບາງບັນຊີຜູ້ໃຊ້)
	$sessionFactory = isset($_SESSION['factory']) ? trim($_SESSION['factory']) : '';
	$sessionNamepro = isset($_SESSION['Namepro']) ? trim($_SESSION['Namepro']) : '';
	$isSanakhamGroup = (
		strcasecmp($sessionFactory, 'Sanakham') === 0 ||
		strcasecmp($sessionFactory, 'HR-Sanakham') === 0 ||
		strcasecmp($sessionNamepro, 'Sanakham') === 0 ||
		strcasecmp($sessionNamepro, 'HR-Sanakham') === 0
	);

	// ===== ກຸ່ມ MuangNan / HR-Luangphabang: dropdown "ຈັດຊື້" ໃຫ້ສະແດງ 2 ຕົວເລືອກຄົງທີ່ນີ້
	// (ຊື້ຢູ່ນະຄອນຫຼວງວຽງຈັນ / ຊື້ຢູ່ເມືອງນານ) ແທນລາຍຊື່ຈາກ tb_by_to =====
	$isMuangNanGroup = (
		strcasecmp($sessionFactory, 'MuangNan') === 0 ||
		strcasecmp($sessionFactory, 'HR-Luangphab') === 0 ||
		strcasecmp($sessionFactory, 'HR-Luangphabang') === 0 ||
		strcasecmp($sessionNamepro, 'MuangNan') === 0 ||
		strcasecmp($sessionNamepro, 'HR-Luangphab') === 0 ||
		strcasecmp($sessionNamepro, 'HR-Luangphabang') === 0
	);
	?>
</head>
			...
			<div class="container-fluid px-4 py-3 bg-soft-blue" style="min-height: 100vh;">
							...
							<div class="item-row">
								...
								<div class="row mt-2">
									...
									<div class="col-12 col-sm-2">
										...
										<input type="text" name="OAout<?= $i ?>" id="OAout<?= $i ?>" class="form-control oaout-input" style="color:blue;" required placeholder="PR ເລກທີ" value="<?= htmlspecialchars($autoOfficeNumber) ?>" />
									</div>
									<div class="col-12 col-sm-2">
									...
									<div class="col-12 col-sm-3">
										<label for="By_to<?= $i ?>" class="fw-bold">
											<i class="fa-solid fa-file-invoice"></i> ຈັດຊື້:
										</label>
										<select name="By_to<?= $i ?>" id="By_to<?= $i ?>" class="form-select" required>
											<option value="">-- ເລືອກ --</option>
											<?php if ($isSanakhamGroup): ?>
												<option value="ຊື້ຢູ່ນະຄອນຫຼວງວຽງຈັນ">ຊື້ຢູ່ນະຄອນຫຼວງວຽງຈັນ</option>
												<option value="ຊື້ຢູ່ປາກລາຍ">ຊື້ຢູ່ປາກລາຍ</option>
											<?php elseif ($isMuangNanGroup): ?>
												<option value="ຊື້ຢູ່ນະຄອນຫຼວງວຽງຈັນ">ຊື້ຢູ່ນະຄອນຫຼວງວຽງຈັນ</option>
												<option value="ຊື້ຢູ່ເມືອງນານ">ຊື້ຢູ່ເມືອງນານ</option>
											<?php else: ?>
												<?php foreach ($byToList as $byTo): ?>
													<option value="<?= htmlspecialchars($byTo['name_of_detail_by']) ?>"><?= htmlspecialchars($byTo['name_of_detail_by']) ?></option>
												<?php endforeach; ?>
											<?php endif; ?>
										</select>
									</div>
				...
				if (isset($_POST["sub"])) {
					...
					$strSQL = "INSERT INTO request 
						(Items,Item_code,Vendor,Use_For,Type,Unit,Groupp,picture,Section,User_Re,Province,dateRe,OA_Out,planing_number,Requirements,Re_requi_persoin,STAFF_ID,Remark_ReQui,By_to,S_Status)
						VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'1')";
					...
					if ($stmt_stock === false || $stmt_ins === false || $stmt_upd_prov === false) {
						...
						for ($i = 1; $i <= $_POST["hdnLine"]; $i++) {
							...
							if (!empty($_POST["cust_id$i"])) {
								...
								if ($test) {
									...
									// ✅ ບໍ່ພົບ Item ນີ້ໃນ stock -> ແຈ້ງເຕືອນຜູ້ໃຊ້ (ກ່ອນໜ້ານີ້ຈະຖືກຂ້າມແບບງຽບໆ ບໍ່ມີການແຈ້ງເຕືອນ
									// ເຮັດໃຫ້ຄິດວ່າບັນທຶກສຳເລັດໝົດ ທັງທີ່ບາງລາຍການບໍ່ຖືກບັນທຶກ)
									$insertOk = false;
									echo '<div class="alert alert-danger">ລາຍການທີ ' . (int)$i . ': ບໍ່ພົບອຸປະກອນ "' . htmlspecialchars($Itemm) . '" ໃນສາງ (ບໍ່ໄດ້ບັນທຶກ)</div>';
								}
							}
						}

						// ✅ ບັນທຶກ (commit) ສະເພາະຕອນທຸກລາຍການຜ່ານໝົດ, ຖ້າມີລາຍການໃດຜິດພາດໃຫ້ rollback ຄືນທັງໝົດ
						// (ກ່ອນໜ້ານີ້ commit() ຖືກເອີ້ນສະເໝີ ເຖິງແມ່ນ $insertOk ຈະເປັນ false ແລ້ວກໍ່ຕາມ
						// ເຮັດໃຫ້ບາງລາຍການຖືກບັນທຶກຄ້າງໄວ້ໃນຖານຂໍ້ມູນ ທັງທີ່ໜ້າຈໍສະແດງ error)
						if ($insertOk) {
							$conn->commit();
						} else {
							$conn->rollback();
						...
						if ($insertOk) {
							// ✅ ຂຶ້ນເລກ PR ອັດຕະໂນມັດ (LS-OFFICE xxx) ແທ້ໆ ສະເພາະຕອນ submit ສຳເລັດ (ຄັ້ງດຽວຕໍ່ 1 ຄັ້ງ submit)
							if ($isVientiane) {
								$fp = fopen($officeCounterFile, 'c+');
								if ($fp) {
									flock($fp, LOCK_EX);
									$current = (int) trim(fread($fp, 1024));
									if ($current <= 0) $current = 373;
									$next = $current + 1;
									ftruncate($fp, 0);
									rewind($fp);
							...
							// ✅ ຂຶ້ນເລກ PR ອັດຕະໂນມັດ (SNKHR001, SNKHR002, ...) ແທ້ໆ ສະເພາະ userlogin = HR-Sanakham
							// ແລະ ສະເພາະຕອນ submit ສຳເລັດ (ຄັ້ງດຽວຕໍ່ 1 ຄັ້ງ submit)
							if ($isHrSanakham) {
								$fpHr = fopen($hrCounterFile, 'c+');
								if ($fpHr) {
									flock($fpHr, LOCK_EX);
									$currentHr = (int) trim(fread($fpHr, 1024));
									if ($currentHr < 0) $currentHr = 0;
									$nextHr = $currentHr + 1;
									ftruncate($fpHr, 0);
									rewind($fpHr);
									fwrite($fpHr, (string) $nextHr);
									fflush($fpHr);
									flock($fpHr, LOCK_UN);
									fclose($fpHr);
								}
							...
							// ✅ ຂຶ້ນເລກ PR ອັດຕະໂນມັດ (MMNHR001, MMNHR002, ...) ແທ້ໆ ສະເພາະ userlogin = HR-Luangphabang
							// ແລະ ສະເພາະຕອນ submit ສຳເລັດ (ຄັ້ງດຽວຕໍ່ 1 ຄັ້ງ submit)
							if ($isHrLuangphabang) {
								$fpMmn = fopen($mmnCounterFile, 'c+');
								if ($fpMmn) {
									flock($fpMmn, LOCK_EX);
									$currentMmn = (int) trim(fread($fpMmn, 1024));
									if ($currentMmn < 0) $currentMmn = 0;
									$nextMmn = $currentMmn + 1;
									ftruncate($fpMmn, 0);
									rewind($fpMmn);
									fwrite($fpMmn, (string) $nextMmn);
									fflush($fpMmn);
									flock($fpMmn, LOCK_UN);
									fclose($fpMmn);
								}
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			function showItemDetails(selectEl, item) {
				...
				if (!item || !selectEl.value) {
					...
					if (oaInputField) {
						oaInputField.value = ''; // ລ້າງຄ່າ ຖ້າຍົກເລີກການເລືອກ
						oaInputField.dataset.userEdited = ''; // ລ້າງ flag ນຳ ເພື່ອໃຫ້ auto-fill ໄດ້ອີກຄັ້ງໃນຄັ້ງໜ້າ
					}
					return;
				}
				...
				// ແຕ່ຖ້າ user ໄດ້ພິມແກ້ໄຂຄ່າໃນຊ່ອງນີ້ດ້ວຍມືມາກ່ອນແລ້ວ (userEdited) ຈະບໍ່ຂຽນທັບຄືນ
				// ເພື່ອໃຫ້ user ຍັງພິມແກ້ໄຂດ້ວຍມືໄດ້ຕະຫຼອດ ເຖິງແມ່ນຈະປ່ຽນ/ຢືນຢັນລາຍການອຸປະກອນອີກຄັ້ງ
				// ✅ ຖ້າລາຍການອຸປະກອນນີ້ບໍ່ມີ OA (oaInput ຫວ່າງ) ຈະ*ບໍ່ລຶບ*ຄ່າເດີມທີ່ມີຢູ່ໃນຊ່ອງ
				// (ເຊັ່ນ ເລກ LS-OFFICE xxx ທີ່ auto-fill ໄວ້ໃຫ້ຕັ້ງແຕ່ຕົ້ນ) - ແກ້ບັນຫາ "ຄົ້ນຫາແລ້ວ ແຕ່ PR ບໍ່ສະແດງ"
				if (oaInputField && oaInputField.dataset.userEdited !== 'true' && oaInput !== '') {
			...
			// ✅ ຕິດຕາມວ່າ user ພິມແກ້ໄຂຊ່ອງ "PR ເລກທີ" ດ້ວຍມືແລ້ວບໍ່
			// ຖ້າພິມແກ້ໄຂແລ້ວ ຈະບໍ່ໃຫ້ auto-fill (ຈາກການເລືອກລາຍການອຸປະກອນ) ຂຽນທັບຄືນ
			document.addEventListener('input', function(e) {
				if (e.target.matches('.oaout-input')) {
